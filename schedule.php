    ...
    <?php if (isset($_GET['action']) && !empty($_GET['action'])) :
      ...
      switch($_GET['action']) {
        ...
        case 'edit' :
          ...
          echo "<p><strong>Lekarz:</strong> {$doctor['degree']} {$doctor['firstname']} {$doctor['lastname']} - {$doctor['specialization']}</p>";
          echo "<p><strong>Interwał wizyt:</strong> co ".$seeingInterval." minut</p>";
          echo "<p><strong>Dzień:</strong> ".str_replace('-','.', $_GET['date'])."</p>";

          $editForm = new Form('edit');
          $editForm->hidden('type', 'edit');
          $editForm->text('start', 'Godzina pierwszej wizyty:', '', 'HH:MM');
          $editForm->text('end', 'Godzina ostatniej wizyty:', '', 'HH:MM');
          $editForm->text('interval', 'Interwał wizyt:', '', 'co ile minut');
          $addForm->select('room', 'Gabinet:', $rooms);
